...
summary_df_desc <- all_high_transacting_details %>%
  group_by(MCC, Description) %>%
...
# Create the bar plot with enhancements
ggplot(summary_df, aes(x = reorder(merchant_category, -count), y = count)) +
  geom_bar(stat = "identity", aes(fill = merchant_category), show.legend = FALSE) +
